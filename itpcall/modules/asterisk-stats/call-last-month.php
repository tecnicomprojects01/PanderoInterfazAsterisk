...
<table width="99%" cellspacing="3" cellpading="3" border="0">
	<caption>TRAFICO MENSUAL</caption>
	<thead>
		<tr>
...
<tbody>
			...
			<tr>
        		<td align="left" ><strong>Select the month</strong></td>
      			<td align="left">
			...
			<tr>
				<td align="left"><strong>Userfield</strong></td>				
			...
			<tr>
				<td align="left"><strong>Accountcode</strong></td>				
...
<? if ($posted==1){ ?>
	<center>
	<IMG SRC="<?php echo base_url();?>asterisk-stats/graph_pie/min_call=<?=$min_call?>&fromstatsday_sday=<?=$fromstatsday_sday?>&months_compare=<?=$months_compare?>&fromstatsmonth_sday=<?=$fromstatsmonth_sday?>&dsttype=<?=$dsttype?>&sourcetype=<?=$sourcetype?>&clidtype=<?=$clidtype?>&channel=<?=$channel?>&resulttype=<?=$resulttype?>&dst=<?=$dst?>&src=<?=$src?>&clid=<?=$clid?>&userfieldtype=<?=$userfieldtype?>&userfield=<?=$userfield?>&accountcodetype=<?=$accountcodetype?>&accountcode=<?=$accountcode?>" ALT="Stat Graph">
	</center>
<? } ?>
</center>
</body>