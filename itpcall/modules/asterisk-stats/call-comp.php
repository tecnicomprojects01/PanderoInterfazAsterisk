...
<body onload="reportes()">
	<FORM METHOD=POST ACTION="<?php echo base_url();?>asterisk-stats/index/s=2&t=<?=$t?>&order=<?=$order?>&sens=<?=$sens?>&current_page=<?=$current_page?>">
	<INPUT TYPE="hidden" NAME="posted" value=1>

<table width="99%" cellspacing="3" cellpading="3" border="0">
	<caption>COMPARACION DE LLAMADAS</caption>
	<thead>
		<tr>
...
<tbody>
			...
			<tr>
        		<td align="left"><b>Select the day</b></td>
      			<td align="left">
			...
			<tr>
				<td align="left"><b>Destination</td>				
			...
			<td align="left"><b>Channel</b></td>				
				...
				<td align="left" colspan="4">
					...
					<option value=1 <? if ($min_call==1){ echo "selected";}?>>Minutes by hours</option>
					<option value=0 <? if (($min_call==0) || !isset($min_call)){ echo "selected";}?>>Number of calls by hours</option>
					</select>
				</td>
			</tr>
...
<tbody><tr><td bgcolor="#000000">			
	...
	<br>
 	<IMG SRC="<?php echo base_url();?>asterisk-stats/graph_stat/min_call=<?=$min_call?>&fromstatsday_sday=<?=$fromstatsday_sday?>&days_compare=<?=$days_compare?>&fromstatsmonth_sday=<?=$fromstatsmonth_sday?>&dsttype=<?=$dsttype?>&sourcetype=<?=$sourcetype?>&clidtype=<?=$clidtype?>&channel=<?=$channel?>&resulttype=<?=$resulttype?>&dst=<?=$dst?>&src=<?=$src?>&clid=<?=$clid?>&userfieldtype=<?=$userfieldtype?>&userfield=<?=$userfield?>&accountcodetype=<?=$accountcodetype?>&accountcode=<?=$accountcode?>" ALT="Stat Graph">
